...
idP:number=0;
succ:boolean=false;
  constructor(private pacserv:PacksService, private frmBuilder:FormBuilder,private actR:ActivatedRoute) { }

  ngOnInit(): void {
    this.idP=this.actR.snapshot.params['id'];
  this.pacserv.getPacksHttp().subscribe(data=>this.pacc=data);
this.pac=this.pacc.find(e=>e.id==this.idP);
  this.pacserv.getpackIdHttp(this.idP).subscribe(data=>this.pack=data);
  //console.log(this.pacc.length);
  ...
  onSub(){
    ...
    this.pacserv.modifPackHttp(this.pack,this.idP).subscribe(data => console.log(data));
  }
